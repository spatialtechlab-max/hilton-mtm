...
import { ArrowLeft, RefreshCw, ChevronDown, ChevronRight, Search, AlertCircle, Code2 } from "lucide-react";
...
/** Render any field value as readable text, expanding arrays/objects. */
...
export default function AdminErpPage() {
  ...
  const matchItem = useCallback((it: ErpItem) => {
  ...
  const visibleCategories = useMemo(() => {
    ...
    return data.categories.filter(
      (c) => c.categoryName.toLowerCase().includes(query) || (itemsByCat.get(c.categoryID) ?? []).some(matchItem),
    );
  }, [data, query, itemsByCat, matchItem]);
  ...
  const catOpen = (id: number) => (query ? true : openCats.has(id));

  if (loading || admin === null) return <Shell><p className="text-eyebrow text-[var(--color-charcoal-500)]">Loading…</p></Shell>;
  if (!user)  return <Shell><p>Sign in with an admin email to continue.</p></Shell>;
  if (!admin) return <Shell><p>{user.email} isn&rsquo;t authorised for the admin panel.</p></Shell>;
    ...
    <Shell>
      <header className="mb-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <span className="text-eyebrow text-[var(--color-burgundy-700)]">Admin · ERP</span>
            ...
            <p className="mt-3 text-[0.9rem] text-[var(--color-charcoal-500)] max-w-2xl">
              The complete live feed from the ERP — every item and category, every field, active and inactive,
              nothing filtered. Fetched fresh on each load.
            </p>
          </div>
          <button
            type="button"
      ...
      {loadingData && !data ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
            ...
            <Stat label="Inactive" value={data.counts.inactive} />
          ...
          <div className="border-y border-black/10 divide-y divide-black/10">
            {visibleCategories.length === 0 && (
              <p className="py-6 text-[0.85rem] text-[var(--color-charcoal-500)]">No categories match “{q}”.</p>
            )}
            {visibleCategories.map((cat) => {
              const items = (itemsByCat.get(cat.categoryID) ?? []).filter(matchItem);
                ...
                <section key={cat.categoryID}>
                  ...
                  >
                    ...
                    <span className="text-[0.75rem] text-[var(--color-charcoal-500)] tabular-nums whitespace-nowrap">
                      {query ? `${items.length} / ` : ""}{cat.count} item{cat.count === 1 ? "" : "s"}
                      <span className="text-[var(--color-charcoal-400)]"> · {cat.active} active · {cat.inactive} inactive</span>
                    </span>
                  </button>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border border-black/10 px-4 py-3">
      ...
      <div className="text-display text-[1.6rem] text-[var(--color-charcoal-900)] tabular-nums mt-1">{value}</div>
    </div>
  );
}
...
function ItemCard({ item, jsonOpen, onToggleJson }: { item: ErpItem; jsonOpen: boolean; onToggleJson: () => void }) {
  ...
  return (
    <div className="border border-black/10 bg-[var(--color-ivory-100)]/40">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-black/10">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className={`text-eyebrow px-2 py-0.5 ${active ? "text-[var(--color-burgundy-700)] bg-[var(--color-burgundy-50)]" : "text-[var(--color-charcoal-500)] bg-black/5"}`}>
  ...
  return (
    <div className="pt-32 md:pt-40 pb-24 min-h-[80vh]">
      <div className="container-editorial">
        <Link href="/admin" className="inline-flex items-center gap-2 text-eyebrow text-[var(--color-charcoal-500)] hover:text-[var(--color-burgundy-700)] transition-colors mb-8">
